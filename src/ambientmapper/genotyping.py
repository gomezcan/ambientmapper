#!/usr/bin/env python3
"""
ambientmapper genotyping — posterior-aware merge → per-cell genotype calls
...
Outputs (current simplified design)
  - <outdir>/<sample>_cells_calls.tsv.gz
      One row per barcode with:
        call, genome_1, genome_2, alpha, rho,
        purity, minor, bic_best, bic_single, bic_doublet, delta_bic,
        n_reads, n_effective, concordance,
        p_top1, p_top2, p_top3, top3_sum, entropy,
        top_genome, best_genome, status_flag, notes, ...
  - <outdir>/<sample>_BCs_PASS_by_mapping.csv
      Legacy-compatible summary: barcode, AssignedGenome (=best_genome),
      call, purity, n_reads.
...
Notes
  ...
  * The likelihood is a pragmatic composite built on per-read soft posteriors
    from `assign`, tuned for model selection and simple downstream use.
"""
...
# Optional imports for QC (currently unused but kept for future use)
try:
...
class MergeConfig(BaseModel):
    ...
    w_nm: float = 0.25                # weight for NM (lower better, so penalty)
    ambient_const: float = 1e-3       # per-read ambient mass before renorm
    p_eps: float = 1e-3               # floor for p-value penalty
    ...
    tau_drop: float = 8.0             # (unused now) posterior-odds threshold for drops
    ...
    sample: str = "sample"
    shards: int = 500                 # number of barcode-hash shards
    chunk_rows: int = 3_000_000       # streaming chunk size for input TSVs
...
def _iter_input_chunks(files: Sequence[Path], chunk_rows: int) -> Iterator[pd.DataFrame]:
    for fp in files:
        try:
            it = pd.read_csv(
                fp,
...
def _concat_worker_shards(worker_dirs: Sequence[Path], final_dir: Path, shards: int) -> None:
...
def genotyping(
    ...
    """Run the two-pass pipeline: posterior calc → ambient estimate → per-cell calls."""
