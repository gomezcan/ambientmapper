...
def assign_winners_for_chunk(filtered_dir: Path, chunk_file: Path, out_file: Path) -> int:
    ...
    for path in qcs:
        ...
        for c in pd.read_csv(path, sep="\t", chunksize=1_000_000):
            ...
            rows.append(sub[["Read","BC","Genome","AS","MAPQ","NM","XAcount"]])
            
    big = pd.concat(rows, ignore_index=True) if rows else pd.DataFrame(
        columns=["Read","BC","Genome","AS","MAPQ","NM", "XAcount"])
    if big.empty:
        pd.DataFrame(columns=["Read","BC","Genome","AS","MAPQ","NM","XAcount","delta_AS"]).to_csv(out_file, sep="\t", index=False)
        return 0
        
    big = big.sort_values(["Read","AS","MAPQ","NM","Genome"], ascending=[True,False,False,True])
    ...
    top[["Read","BC","Genome","AS","MAPQ","NM", "XAcount","delta_AS","is_tie"]].to_csv(out_file, sep="\t", index=False)
    return int(len(top))
